            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @if($user->two_factor_secret == null)
                        <form class="form-horizontal" method="POST" action="{{ route('HXM2FA.generate2faSecret') }}">
                            {{ csrf_field() }}
                    ...
                    @elseif(!$user->two_factor_enabled)
                        1. Scan this QR code with your Google Authenticator App. @if(config('hxm2fa.show_secret')) Alternatively, you can use the code: <code>{{ $secret_key }}</code>@endif<br/>
                        {!! $google2fa_url !!}
                        <br/><br/>
                        2. Enter the pin from Google Authenticator app:<br/><br/>
                        <form class="form-horizontal" method="POST" action="{{ route('HXM2FA.enable2fa') }}">
                            {{ csrf_field() }}
                    ...
                    @elseif($user->two_factor_enabled)
                        ...
                        <form class="form-horizontal" method="POST" action="{{ route('HXM2FA.disable2fa') }}">
                            {{ csrf_field() }}
